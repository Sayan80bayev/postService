...
func NewPostService(
	...
	cacheService *CacheService,
	producer *messaging.Producer) *PostService {
...
func (ps *PostService) UpdatePost(content, title string, userId, postId, categoryID uint, file multipart.File, header *multipart.FileHeader, cfg *config.Config) error {
...
func (ps *PostService) DeletePost(postId, userId uint) error {
	post, err := validatePermission(userId, postId, ps)
	if err != nil {
		logger.Errorf("Permission denied for user %d on post %d", userId, postId)
		return err
...
func validatePermission(userId, postId uint, ps *PostService) (*models.Post, error) {
